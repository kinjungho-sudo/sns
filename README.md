...
![미리보기](out/slide-01.png)

## 폴더 구조

| 파일 | 역할 |
|------|------|
| `slides.js`   | **여기만 고치면 됩니다.** 슬라이드 내용 + 템플릿 |
| `styles.css`  | 디자인 시스템(색·폰트·레이아웃) |
| `index.html`  | 브라우저 미리보기 (← → 방향키로 이동) |
| `render.html` | PNG 추출용 렌더 페이지 (직접 열 필요 없음) |
| `export.mjs`  | 각 슬라이드를 `out/slide-NN.png` 로 저장 |
...
npm run export       # out/ 폴더에 PNG 저장 (기본 2160×2700, 고해상도)
```
...
`slides.js` 상단의 `SLIDES` 배열만 수정하세요. 각 항목의 `type` 으로 레이아웃을 고릅니다.
